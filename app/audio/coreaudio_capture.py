...
_LOOPBACK_NAMES = ("blackhole", "loopback", "soundflower", "multi-output")


class CoreAudioCapture(AudioCapture):
    """macOS CoreAudio loopback capture.

    Requires BlackHole (2ch or 16ch) or similar virtual audio device installed.
    Falls back to the default microphone with a warning if none found.
    """
    ...
    def start(self) -> None:
        ...
        self._event_bus.status_changed.emit("Recording (CoreAudio loopback)...")
    ...
    def _find_loopback_device(self, soundcard):
        all_mics = soundcard.all_microphones(include_loopback=True)
        for mic in all_mics:
            if any(name in mic.name.lower() for name in _LOOPBACK_NAMES):
                return mic
        self._event_bus.status_changed.emit(
            "Warning: No loopback device found (install BlackHole). Using default mic."
        )
    ...
    def _capture_loop(self) -> None:
        ...
        try:
            device = self._find_loopback_device(soundcard)
            ...
            self._event_bus.error_occurred.emit("audio", f"CoreAudio error: {exc}")
            self._running = False
